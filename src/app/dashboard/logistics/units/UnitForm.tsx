...
function UnitForm({ modal, unit, setUnit, fetchUnits, initialState, accessToken }: any) {
    ...
    const handleSaveSubLine = async (e: FormEvent<HTMLFormElement>) => {
        ...
        if (Number(unit.id) !== 0) {
            queryFetch = `
                mutation{
                    updateUnit(
                        id:${unit.id}, shortName: "${unit.shortName}", description: "${unit.description}", code: "${unit.code}"
                    ){
                        message
                    }
                }
            ...
            queryFetch = `
                mutation{
                    createUnit(
                        shortName: "${unit.shortName}", description: "${unit.description}", code: "${unit.code}"
                    ){
                        message
                    }
                }
