...
function WarehouseForm({ modal, warehouse, setWarehouse, fetchWarehouses, initialState, accessToken }: any) {
        ...
        if (Number(warehouse.id) !== 0) {
            queryFetch = `
                mutation{
                    updateWarehouse(
                        id:${warehouse.id}, 
                        name: "${warehouse.name}", subsidiaryId: ${warehouse.subsidiaryId}, category: "${warehouse.category.replace("A_", "")}"
                    ){
                        message
                    }
                }
                    ...
                    <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                            <h3 className="text-xl font-medium text-gray-900 dark:text-white">
                                {Number(warehouse.id) > 0 ? "Editar" : "Registrar nuevo almacen"}
                            </h3>
                        ...
                        <form onSubmit={handleSaveWarehouse}>
                            <div className="p-4 md:p-5 space-y-4">
                                ...
                                <div className="grid">
                                    <div className="sm:col-span-2 mb-4">
                                        <label htmlFor="name" className="form-label-sm">Nombre</label>
                                        <input type="text" name="name" id="name" maxLength={45} value={warehouse.name || ""} onChange={handleInputChange} onFocus={(e) => e.target.select()} className="form-control-sm" required autoComplete="off" />
                                    </div>

                                    <div className="sm:col-span-2 mb-4">
                                        <label htmlFor="subsidiaryId" className="form-label-sm">Sede</label>
                                        <select name="subsidiaryId" id="subsidiaryId" onChange={handleInputChange} value={warehouse.subsidiaryId} className="form-control-sm" required>
                                    ...
                                    <div className="sm:col-span-2">
                                        <label htmlFor="category" className="form-label-sm">Categoria</label>
                                        <select name="category" id="category" onChange={handleInputChange} value={warehouse.category.replace("A_", "")} className="form-control-sm" required>
                            ...
                            <div className="flex p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600  justify-end">
                                <button id="btn-save" type="submit" className="btn-green px-5 py-2 inline-flex items-center gap-2">
                                    <Save />{Number(warehouse.id) > 0 ? "Actualizar" : "Guardar"}
                                </button>
                            </div>
                        </form>
