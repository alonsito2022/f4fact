...
function SaleTotalList({
  ...
  useEffect(() => {
    if (invoice?.perceptionType) {
      ...
      let totalPerception = Number(invoice?.totalAmount) * perceptionPercentage;
  ...
  useEffect(() => {
    if (invoice?.retentionType) {
      ...
      let totalRetention = Number(invoice?.totalAmount) * retentionPercentage;
      setSale((prevSale: IOperation) => ({
        ...prevSale,
                      ...
                      <div className="sm:col-span-2">
                        ...
                        >
                          {detractionPaymentMethodsData?.allDetractionPaymentMethods?.map(
                            (o: IDetractionPaymentMethod, k: number) => (
                              <option key={k} value={o.code}>
                                {o.name}
                              </option>
                            )
