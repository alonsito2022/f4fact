...
function NewSalePage() {
    ...
    const handleProduct = (event: ChangeEvent<HTMLSelectElement | HTMLInputElement | HTMLTextAreaElement>) => {
        const { name, value } = event.target;
    ...
    function calculateTotal() {
        const discountForItem = sale?.operationdetailSet?.reduce((total: number, detail: IOperationDetail) => {
        ...
        const totalUnaffected = sale?.operationdetailSet?.filter((detail: IOperationDetail) => detail.typeAffectationId == 3).reduce((total: number, detail: IOperationDetail) => {
            return total + Number(detail.totalValue);
        }, 0);

        const totalExonerated = sale?.operationdetailSet?.filter((detail: IOperationDetail) => detail.typeAffectationId == 2).reduce((total: number, detail: IOperationDetail) => {
            return total + Number(detail.totalValue);
        }, 0);

        const totalTaxed = sale?.operationdetailSet?.filter((detail: IOperationDetail) => detail.typeAffectationId == 1).reduce((total: number, detail: IOperationDetail) => {
            return total + Number(detail.totalValue);
        }, 0);

        const totalIgv = sale?.operationdetailSet?.reduce((total: number, detail: IOperationDetail) => {
    ...
    return (
        <>
                            ...
                            <div className="p-4 md:p-5 space-y-4">
                                ...
                                <div className="grid gap-2 grid-cols-4">
                                    ...
                                    <div className="sm:col-span-1">
                                        ...
                                        <select value={sale.igvType} name="igvType" onChange={handleSale} className="form-control-sm" required>
                                    ...
                                    <div className="sm:col-span-2">
                                        ...
                                        <select value={sale.documentType} name="documentType" onChange={handleSale} className="form-control-sm" required>
                                ...
                                <div className="grid gap-2 grid-cols-4">
                                    ...
                                    <div className="sm:col-span-1">
                                        ...
                                        <select value={sale.currencyType} name="currencyType" onChange={handleSale} className="form-control-sm">
                                ...
                                <div className="flex justify-end py-2">
                                    <button type="button" className="btn-blue px-5 py-2 inline-flex items-center gap-2" onClick={(e) => {
                                        modalAddDetail.show(); setSaleDetail(initialStateSaleDetail);
                                    }}><Add /> AGREGAR ITEM
                                    </button>
                                </div>
                                ...
                                <div className="overflow-hidden shadow">
                                    <table className="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                                        <thead className="bg-gray-100 dark:bg-gray-700">
                                            <tr>
                                                <th scope="col" className="p-3 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">Descripción</th>
                                                <th scope="col" className="p-3 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">Cantidad</th>
                                                <th scope="col" className="p-3 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">C/U</th>
                                                <th scope="col" className="p-3 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">Subtotal</th>
                                                <th scope="col" className="p-3 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400"></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {sale?.operationdetailSet?.map((item: any, i: number) =>
                                                    ...
                                                    <td className="px-4 py-2">
                                                        <div className="flex justify-end py-2">
                                                            ...
                                                            <a className="hover:underline cursor-pointer" onClick={(e) => {
                                                                modalAddDetail.show(); setSaleDetail(item);
                                                                setProduct({ ...product, id: Number(item.productId), name: item.productName });
                                                            }}>
                                                                <Edit />
                                                            </a>

                                                            <a className="hover:underline cursor-pointer" onClick={() => handleRemoveSaleDetail(Number(item?.temporaryId))}>
                                                                <Delete />
                                                            </a>
                                ...
                                <div className="grid grid-cols-4">
                                    <div className="sm:col-span-3 text-right">% Descuento Global</div><div className="sm:col-span-1 text-right">0.00</div>
                                    <div className="sm:col-span-3 text-right">Descuento Global (-) {sale.currencyType === "PEN" ? "S/" : sale.currencyType === "USD" ? "US$" : sale.currencyType === "EUR" ? "€" : sale.currencyType === "GBP" ? "£" : null}</div><div className="sm:col-span-1 text-right">{sale.discountGlobal}</div>
                                    <div className="sm:col-span-3 text-right">Descuento por Item (-) {sale.currencyType === "PEN" ? "S/" : sale.currencyType === "USD" ? "US$" : sale.currencyType === "EUR" ? "€" : sale.currencyType === "GBP" ? "£" : null}</div><div className="sm:col-span-1 text-right">{sale.discountForItem}</div>
                                    <div className="sm:col-span-3 text-right">Descuento Total (-) {sale.currencyType === "PEN" ? "S/" : sale.currencyType === "USD" ? "US$" : sale.currencyType === "EUR" ? "€" : sale.currencyType === "GBP" ? "£" : null}</div><div className="sm:col-span-1 text-right">{sale.totalDiscount}</div>
                                    <div className="sm:col-span-3 text-right">Exonerada {sale.currencyType === "PEN" ? "S/" : sale.currencyType === "USD" ? "US$" : sale.currencyType === "EUR" ? "€" : sale.currencyType === "GBP" ? "£" : null}</div><div className="sm:col-span-1 text-right">{sale.totalExonerated || "0.00"}</div>
                                    <div className="sm:col-span-3 text-right">Inafecta {sale.currencyType === "PEN" ? "S/" : sale.currencyType === "USD" ? "US$" : sale.currencyType === "EUR" ? "€" : sale.currencyType === "GBP" ? "£" : null}</div><div className="sm:col-span-1 text-right">{sale.totalUnaffected || "0.00"}</div>
                                    <div className="sm:col-span-3 text-right">Gravada {sale.currencyType === "PEN" ? "S/" : sale.currencyType === "USD" ? "US$" : sale.currencyType === "EUR" ? "€" : sale.currencyType === "GBP" ? "£" : null}</div><div className="sm:col-span-1 text-right">{sale.totalTaxed}</div>
                                    <div className="sm:col-span-3 text-right">IGV {sale.currencyType === "PEN" ? "S/" : sale.currencyType === "USD" ? "US$" : sale.currencyType === "EUR" ? "€" : sale.currencyType === "GBP" ? "£" : null}</div><div className="sm:col-span-1 text-right">{sale.totalIgv}</div>
                                    <div className="sm:col-span-3 text-right">Gratuita {sale.currencyType === "PEN" ? "S/" : sale.currencyType === "USD" ? "US$" : sale.currencyType === "EUR" ? "€" : sale.currencyType === "GBP" ? "£" : null}</div><div className="sm:col-span-1 text-right">{sale.totalFree || "0.00"}</div>
                                    <div className="sm:col-span-3 text-right">Otros Cargos {sale.currencyType === "PEN" ? "S/" : sale.currencyType === "USD" ? "US$" : sale.currencyType === "EUR" ? "€" : sale.currencyType === "GBP" ? "£" : null}</div><div className="sm:col-span-1 text-right">0.00</div>
                                    <div className="sm:col-span-3 text-right">Total {sale.currencyType === "PEN" ? "S/" : sale.currencyType === "USD" ? "US$" : sale.currencyType === "EUR" ? "€" : sale.currencyType === "GBP" ? "£" : null}</div><div className="sm:col-span-1 text-right">{sale.totalAmount}</div>
                                </div>
                                <div className="flex justify-end py-2">
                                    <button type="button"
                                        className={`btn-blue px-5 py-2 inline-flex items-center gap-2 ${sale?.operationdetailSet?.length === 0 ? "cursor-not-allowed" : ""}`}
                                        onClick={async () => {
                                            if (Number(sale.clientId) === 0) { toast('Por favor ingrese un cliente.', { hideProgressBar: true, autoClose: 2000, type: 'warning' }); return; }
                                            if (sale.operationdetailSet.length === 0) { toast('Por favor ingrese al menos un item.', { hideProgressBar: true, autoClose: 2000, type: 'warning' }); return; }
                                            modalWayPay.show();
                                            setSale({ ...sale, totalPayed: "", cashflowSet: [] })
                                            setCashFlow({ ...cashFlow, total: Number(sale.totalToPay) });
                                        }} disabled={sale?.operationdetailSet?.length === 0}><Save />CONTINUAR CON EL PAGO
                                    </button>
                                </div>
                            </div>
            ...
            <ProductForm modalProduct={modalProduct} setModalProduct={setModalProduct} product={product} setProduct={setProduct} jwtToken={jwtToken} initialStateProduct={initialStateProduct} typeAffectationsData={typeAffectationsData} PRODUCTS_QUERY={PRODUCTS_QUERY} productFilterObj={productFilterObj} />
            <SaleDetailForm modalAddDetail={modalAddDetail} setModalAddDetail={setModalAddDetail} product={product} setProduct={setProduct} saleDetail={saleDetail} setSaleDetail={setSaleDetail} sale={sale} setSale={setSale}
                jwtToken={jwtToken} initialStateProduct={initialStateProduct} initialStateSaleDetail={initialStateSaleDetail} typeAffectationsData={typeAffectationsData} productsData={productsData} />
            <WayPayForm modalWayPay={modalWayPay} setModalWayPay={setModalWayPay} cashFlow={cashFlow} setCashFlow={setCashFlow} initialStateCashFlow={initialStateCashFlow} initialStateSale={initialStateSale} sale={sale} setSale={setSale} jwtToken={jwtToken} wayPaysData={wayPaysData} />
        </>
    )
}
