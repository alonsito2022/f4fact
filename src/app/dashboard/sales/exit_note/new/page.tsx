...
import ExitNoteTotalList from "./ExitNoteTotalList";
    ...
    useEffect(() => {
        if (serialsAssignedData?.allSerials?.length > 0) {
            const filteredSeries = serialsAssignedData.allSerials.filter(
                (s: ISerialAssigned) =>
                    s.documentType.replace("A_", "") === sale.documentType &&
                    !s.isGeneratedViaApi
            );
...
export default ExitNoteNewPage;
