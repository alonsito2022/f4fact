...
function SubsidiaryPage() {
    ...
    useEffect(() => {
      if (searchTerm.length >= 3) {
          fetchSubsidiariesbyName(); // Realizar la llamada a la API cuando el término de búsqueda tiene al menos 3 caracteres
      }
      else{
          fetchSubsidiaries()
      }
...
<nav className="flex justify-between mb-3 content-center px-5 py-3 text-gray-700 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-800 dark:border-gray-700" aria-label="Breadcrumb">
  <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
    ...
    <li aria-current="page">
      ...
      <SubsidiaryModal modal={modal}  setModal={setModal} subsidiary={subsidiary} setSubsidiary={setSubsidiary} initialState={initialState} fetchSubsidiaries={fetchSubsidiaries}/>
    </>
  )
}
